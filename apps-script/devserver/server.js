 ...
 * los mocks de SpreadsheetApp/Utilities/Session/LockService — la única
 * diferencia con producción es el transporte: aquí el cliente habla
 * HTTP/fetch en vez de google.script.run (ver Client_Api.html → callServer()).
 *
 * Uso como CLI: node devserver/server.js [--port 8080]
 * Uso como módulo (specs e2e, apps-script/e2e/): createDevServer({ port: 0 })
 * devuelve { server, port, ctx, close() } con un backend nuevo y aislado
 * (puerto 0 = el sistema operativo asigna uno libre, para poder correr
 * varias instancias en paralelo sin chocar).
 */
...
module.exports = { createDevServer, DIRECT_EMAIL, SHARED_EMAIL };

if (require.main === module) {
  const portFlagIndex = process.argv.indexOf('--port');
  const cliPort = portFlagIndex !== -1 ? Number(process.argv[portFlagIndex + 1]) : 8080;
  createDevServer({ port: cliPort }).then((dev) => {
    console.log('Dev server escuchando en http://localhost:' + dev.port);
    console.log('Por defecto: DIRECT sin PIN, como Eduardo (' + DIRECT_EMAIL + ').');
    console.log('Para probar el flujo SELECT_PIN+PIN: http://localhost:' + dev.port + '/?email=' + encodeURIComponent(SHARED_EMAIL));
