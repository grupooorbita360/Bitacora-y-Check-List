...
| **3** | Tasks (CRUD, permisos, assignment, lifecycle, history, comments, participants, subtasks, adjustments, bulk reassignment) **+ Checklist** (`Checklist_Config`, `Checklist_Runs`, conversión a Task) | ✅ Código y tests entregados en `apps-script/`, con las 87 reglas reales de `Task_Permissions` y las 19 de `Task_Assignment_Config` ya sembradas (ver `06-fase-3-decisiones.md`). Pendiente de instalar en el Sheet real y validar. |
